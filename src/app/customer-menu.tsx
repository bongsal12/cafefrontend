...
export default function CustomerMenuPage() {
  ...
  const [payMethod, setPayMethod] = useState<"cash" | "khqr" | null>(null);
  ...
  useEffect(() => {
    const tableParam = searchParams.get("table_no");
    if (tableParam) {
  ...
  async function proceedWithPayment(method: "cash" | "khqr") {
    setPayMethod(method);
    setSubmitting(true);
    setPayError(null);
    ...
    try {
      ...
      setPayLoading(false);
      setPayError(e?.message || "Place order failed");
      setToast(e?.message || "Place order failed");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#059669]">
      ...
      >
        ...
        {cartCount > 0 ? (
          <span className="absolute -top-2 -right-2 min-w-5.5 rounded-full bg-red-600 px-2 py-0.5 text-center text-xs font-extrabold leading-none">
            {cartCount}
          </span>
        ) : null}
      </button>
      ...
      {cartOpen ? (
        <div className="fixed inset-0 z-50">
          ...
          {/* panel */}
          <div className="absolute right-0 top-0 h-full w-full md:max-w-105  bg-[#042f2e] shadow-2xl">
            ...
            <div className="h-[calc(100%-64px)] overflow-y-auto p-6">
              ...
              <div className="mt-4 rounded-2xl border bg-[#059669] border-black/5 p-4">
                ...
                <button
                  ...
                  onClick={placeOrder}
                >
                  {submitting ? "Processing..." : "Choose Payment Method"}
                </button>
              </div>
            </div>
          </div>
      ...
      {payOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          {!payMethod ? (
            ...
            <div className="w-full max-w-105 rounded-3xl bg-white p-5 shadow-xl">
              <div className="flex items-start justify-between gap-3 mb-6">
                <div>
                  <div className="text-lg font-extrabold text-[#4f2206]">Payment Method</div>
              ...
              <div className="grid gap-3">
                <button
                  className="rounded-2xl border-2 border-[#059669] bg-[#059669]/10 px-6 py-4 text-center font-extrabold text-[#059669] hover:bg-[#059669] hover:text-white transition"
                  onClick={() => proceedWithPayment("cash")}
                  disabled={submitting}
                >
                  Pay with Cash
                </button>
                ...
                <button
                  className="rounded-2xl border-2 border-[#a71237] bg-[#a71237]/10 px-6 py-4 text-center font-extrabold text-[#a71237] hover:bg-[#a71237] hover:text-white transition"
                  onClick={() => proceedWithPayment("khqr")}
                  disabled={submitting}
                >
                  Pay with KHQR
                </button>
